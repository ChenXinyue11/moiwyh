...
void Board::showBothMi() { int psn; 
  ...
  for (int r = 0; r < N; r++) {
    ...
    for (int c = 0; c < N; c++) { psn = fatten(r,c);
      prMiai(reply[nx(BLK)][psn],psn);
    }
    for (int k = 0; k < 2*N; k++) printf("  ");
    for (int c = 0; c < N; c++) { psn = fatten(r,c);
...
void Board::show() {
  ...
  for (int j = 0; j < N; j++) {
    prtBlanks(j);
    printf("%2d ",j+1);
    for (int k=0; k<N; k++) printf(" %c ",emit(board[fatten(j,k)]));
    prtBlanks(3*(j+1));
    for (int k=0; k<N; k++) prtNonZero(brdr[fatten(j,k)],2);
    printf("\n");
  }
  printf("\n"); }
...
void Board::init() { 
  ...
  for (j=0; j<N; j++) 
    brdr[fatten(0,0)+j*(Np2G)+N] = BRDR_R;
  for (j=0; j<N; j++) 
    brdr[fatten(0,0)+j+(Np2G)*N] = BRDR_BOT;
  printf("lalala\n");
  for (int j=0; j<TotalGBCells; j++) {
            printf("%d",brdr[j]);
        }  
